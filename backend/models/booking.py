...
class BasePrice(Base):
    ...
    __table_args__ = (UniqueConstraint("city_id", "show_period", name="uq_city_period"),)
    ...
    def __repr__(self):
        return (
            f"<BasePrice(city={self.city_id}, period='{self.show_period}', "
            f"lower=£{self.lower_hall_price})>"
        )
...
class Booking(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "show_date <= DATE(booking_date) + INTERVAL 7 DAY", name="chk_advance_booking"
        ),
    ...
    showing_id = Column(
        Integer, ForeignKey("showings.showing_id", ondelete="RESTRICT"), nullable=False, index=True
    )
    ...
    booking_status = Column(
        String(10), nullable=False, default="confirmed", index=True
    )  # confirmed / cancelled
    ...
    booked_seats = relationship(
        "BookedSeat", back_populates="booking", lazy="selectin", cascade="all, delete-orphan"
    )

    def __repr__(self):
        return (
            f"<Booking(booking_id={self.booking_id}, ref='{self.booking_reference}', "
...
class BookedSeat(Base):
    ...
    seat_id = Column(
        Integer, ForeignKey("seats.seat_id", ondelete="RESTRICT"), nullable=False, index=True
    )
    ...
    def __repr__(self):
        return (
            f"<BookedSeat(booking={self.booking_id}, seat={self.seat_id}, "
