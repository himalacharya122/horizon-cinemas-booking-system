...
class Cinema(Base):
    ...
    city_id       = Column(Integer, ForeignKey("cities.city_id", ondelete="RESTRICT"), nullable=False, index=True)
    ...
    screens = relationship("Screen", back_populates="cinema", lazy="selectin", cascade="all, delete-orphan")
...
class Screen(Base):
    ...
    cinema_id           = Column(Integer, ForeignKey("cinemas.cinema_id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    seats    = relationship("Seat", back_populates="screen", lazy="selectin", cascade="all, delete-orphan")
    listings = relationship("Listing", back_populates="screen", lazy="selectin")

    def __repr__(self):
        return f"<Screen(screen_id={self.screen_id}, cinema={self.cinema_id}, number={self.screen_number})>"


class Seat(Base):
    __tablename__ = "seats"
    __table_args__ = (
        UniqueConstraint("screen_id", "seat_number", name="uq_seat"),
    )
    ...
    def __repr__(self):
        return f"<Seat(seat_id={self.seat_id}, number='{self.seat_number}', type='{self.seat_type}')>"