...
class Listing(Base):
    ...
    __table_args__ = (CheckConstraint("end_date >= start_date", name="chk_listing_dates"),)

    listing_id = Column(Integer, primary_key=True, autoincrement=True)
    film_id = Column(
        Integer, ForeignKey("films.film_id", ondelete="CASCADE"), nullable=False, index=True
    )
    screen_id = Column(
        Integer, ForeignKey("screens.screen_id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    showings = relationship(
        "Showing", back_populates="listing", lazy="selectin", cascade="all, delete-orphan"
    )

    def __repr__(self):
        return (
            f"<Listing(listing_id={self.listing_id}, film={self.film_id}, screen={self.screen_id})>"
        )
...
class Showing(Base):
    ...
    listing_id = Column(
        Integer, ForeignKey("listings.listing_id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    def __repr__(self):
        return (
            f"<Showing(showing_id={self.showing_id}, time={self.show_time}, "
            f"type='{self.show_type}')>"
        )
