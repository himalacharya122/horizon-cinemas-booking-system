...
class HelpView(QWidget):
    ...
    def _build_guide_tab(self) -> QScrollArea:
        ...
        for title, text in sections:
            ...
            content.setStyleSheet(f"color: {TEXT_SECONDARY}; background: transparent; line-height: 1.6;")
    ...
    def _build_contact_tab(self) -> QScrollArea:
        ...
        info = [
            ...
            ("For account issues:", "Password resets and account changes must be\nrequested through your cinema administrator."),
            ("Emergency support:", "Contact the Horizon Cinemas IT help desk\nat support@horizoncinemas.co.uk"),
        ]

        for label, value in info:
            lbl = QLabel(label)
            lbl.setFont(body_font(10))
            lbl.setStyleSheet(f"color: {TEXT_MUTED}; background: transparent; font-weight: 600; margin-top: 8px;")
            card.add(lbl)
