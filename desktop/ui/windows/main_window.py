...
from PyQt6.QtCore import Qt, QSize, QPropertyAnimation, QEasingCurve  # type: ignore
...
from desktop.ui.theme import (
    ACCENT, ACCENT_LIGHT, WHITE,
    BG_DARKEST, BG_DARK, BG_CARD, BG_HOVER, BORDER, BORDER_LIGHT,
...
class CollapsibleSection(QWidget):
    ...
    def __init__(self, title: str):
        ...
        self.header.setFont(body_font(8))
        self.header.setCursor(Qt.CursorShape.PointingHandCursor)
        self.header.setFixedHeight(32)
        self.header.setStyleSheet(
            f"QPushButton {{ text-align: left; color: {TEXT_MUTED}; "
            f"font-weight: 700; letter-spacing: 1px; "
            f"padding: 8px 16px 4px 16px; border: none; "
...
class MainWindow(QWidget):
    ...
    def _build_ui(self):
        ...
        brand_frame.setStyleSheet(
            f"border-bottom: 1px solid {BORDER}; background: {BG_DARK};"
        )
        bl = QVBoxLayout(brand_frame)
        bl.setContentsMargins(16, 8, 16, 8)
        brand = QLabel("HORIZON CINEMAS")
        brand.setFont(heading_font(10))
        brand.setStyleSheet(f"color: {ACCENT}; background: transparent; border: none; letter-spacing: 1px;")
        bl.addWidget(brand)
        ...
        scroll.setStyleSheet(
            ...
            f"QScrollBar:vertical {{ width: 6px; background: {BG_DARK}; }}"
        ...
        user_frame.setStyleSheet(
            f"border-top: 1px solid {BORDER}; background: {BG_DARKEST};"
        )
        ...
        name_lbl.setStyleSheet(f"color: {TEXT_PRIMARY}; background: transparent; font-weight: 600; border: none;")
        uf.addWidget(name_lbl)

        role_lbl = QLabel(api.role.replace("_", " ").title())
        role_lbl.setFont(body_font(9))
        role_lbl.setStyleSheet(f"color: {TEXT_MUTED}; background: transparent; border: none; font-weight: 500;")
